...
# Ваши реквизиты:
PAYMENT_DETAILS = "по номеру +7908-328-86-84 (Сбербанк, Анна Викторовна В.)"
...
PAYMENT_CARD_PATH = "payment_card.png"
...
async def question_received(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    confirm_text = (
        ...
        f"❓ Вопрос: {question}\n\n"
        f"💳 Для оплаты переведи {package['price']} {PAYMENT_DETAILS}.\n"
        f"После оплаты разбор будет готов в течение дня 🔐"
    )

    # Пробуем прикрепить картинку, если она есть
    photo_sent = False
    if os.path.exists(PAYMENT_CARD_PATH):
        try:
            with open(PAYMENT_CARD_PATH, "rb") as card_file:
    ...
    # Если фото не отправилось (или файла нет), отправляем текстом
    if not photo_sent:

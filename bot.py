...
# Запасной file_id вашей карточки оплаты в Telegram
PAYMENT_CARD_FILE_ID = "AgACAgIAAxkBAAEu5LtqrQU6aq-OGp6pQZdCQHz-yCoC5AACCx5rGxK2aElD1TJ_CeFxsAEAAwIAA3kAAz0E"
...
async def question_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # 1) Клиенту — подтверждение и реквизиты для оплаты
    confirm_text = (
        f"Спасибо! Заявка принята:\n\n"
        f"📦 {package['title']} — {package['price']}\n"
        f"❓ Вопрос: {question}\n\n"
        f"Для оплаты переведи {package['price']} по номеру телефона "
        f"(реквизиты на карточке ниже). После оплаты разбор будет готов "
        f"в течение дня 🔐"
    )
    await update.message.reply_text(confirm_text, reply_markup=ReplyKeyboardRemove())

    # Сначала пробуем отправить локальный файл, если его нет — отправляем по file_id
    try:
        if os.path.exists(PAYMENT_CARD_PATH):
            with open(PAYMENT_CARD_PATH, "rb") as f:
                await update.message.reply_photo(photo=f)
        else:
            await update.message.reply_photo(photo=PAYMENT_CARD_FILE_ID)
